...
def process_book_to_codex(filepath, explicit_pg_id=None):
    ...
    # Prefer curated metadata from v3; generate if missing
    metadata_filename = os.path.splitext(filename)[0] + ".metadata.json"
    metadata_path = os.path.join(METADATA_DIR, metadata_filename)
    if os.path.exists(metadata_path):
        with open(metadata_path, 'r', encoding='utf-8') as f:
            curated_metadata = json.load(f)
        merged_exclusions = curated_metadata.get('exclusions', [])
        curator_model = curated_metadata.get('model', MODEL_NAME)
        print(f"   📑 Using existing exclusions from {metadata_filename}")
    else:
        curated_metadata = process_book(filepath)
        merged_exclusions = curated_metadata.get('exclusions', [])
        curator_model = curated_metadata.get('model', MODEL_NAME)
    ...
    metadata['curation']['model']['name'] = curator_model
    ...
    if os.path.exists(curated_path):
        with open(curated_path, 'r', encoding='utf-8') as f:
            clean_text = f.read()
        print(f"   🧹 Using curated clean text from {curated_path}")
    else:
        clean_text = extract_clean_text(raw_text, merged_exclusions)
        print("   🧹 Clean text generated inline (no curated file found)")
    ...
    if os.path.exists(codex_path):
        print(f"   ✅ CODEX already exists at {codex_path}, skipping")
        return
